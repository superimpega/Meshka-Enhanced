...
//
addGlobalStyle('html{background: url("http://i.imgur.com/Rqy6I.jpg") no-repeat scroll center top #000000;');
//
...
//addGlobalStyle('#dj-console, #dj-console {background-image: url("http://i.imgur.com/I0E5L.png");min-height:33px;min-width:131px;}'); //change create room button
...
function strobeSwap()
{
	...
	if (strobeState) strobeID = setInterval(strobe, 150);
}
...
function appendUser(user) 
{
	...
	switch (vote) 
	{
		case 1:		// WOOT!
			...
			points++;
			if (bouncer)
				img = "http://i.imgur.com/S5Kep.png";
			if (host)
				img = "http://i.imgur.com/Cr5Xp.png";
			if (manager)
				img = "http://i.imgur.com/nVbYy.png";
			break;
		case 0:		// Undecided
			colour = "FFFFFF";
			if (bouncer) 
	 ...
	 */
	if (img == undefined && (bouncer || host || ambassador)) 
	{
		colour = "FFFFFF";
	...
	$('#plugbot-userlist').append(
		((bouncer || host || ambassador) ? '<img src="' + img + '" align="left" style="margin-left:6px" alt="Moderator" />' : '') 
...
//autochat
/*
function isBoris() { return API.getSelf().username == "Meshka"; }

if (isBoris())
{
    window.setInterval(function() {
    API.sendChat("Check out our Room :) ");
    }, (1000 * 60 * 60));
}
*/
